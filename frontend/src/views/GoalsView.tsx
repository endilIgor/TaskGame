...
export function GoalsView() {
  ...
  async function advance(goal: Goal) { setBusyId(goal.id); setError(null); try { await apiPost<Mission, MissionProgressPayload>(`/missions/${goal.id}/progress`, { amount: 1 }); setRefreshKey((key) => key + 1); } catch (cause) { setError(cause instanceof Error ? cause.message : "Nao foi possivel atualizar o objetivo."); } finally { setBusyId(null); } }
  return <section className="view-page" aria-labelledby="goals-heading"><header><span className="section-kicker">Objetivos</span><h1 className="page-heading" id="goals-heading">Campanhas</h1></header>{error ? <ErrorPanel>{error}</ErrorPanel> : null}{goals.status === "loading" ? <LoadingPanel /> : null}{goals.status === "error" ? <ErrorPanel>{goals.error}</ErrorPanel> : null}{goals.status === "ready" ? <div className="goal-list">{goals.data.length ? goals.data.map((goal) => <article className="goal-row" key={goal.id}><MissionCard mission={goal} /><ProgressBar value={goal.progress_current} max={goal.progress_target ?? 1} label={`${goal.progress_percent}% concluido`} />{goal.status === "active" ? <button className="button primary" type="button" disabled={busyId === goal.id} onClick={() => advance(goal)}>Avancar +1</button> : <span className="badge-status">Concluido</span>}</article>) : <EmptyState>Nenhuma campanha ativa.</EmptyState>}</div> : null}</section>;
}
