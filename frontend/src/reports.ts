...
function stat(label: string, value: string, valueClassName?: string): HTMLElement {
...
function createChart(report: WeeklyReport): HTMLElement {
...
function createAnalysis(report: WeeklyReport): HTMLElement {
...
function reportPage(root: HTMLElement, report: WeeklyReport, isCurrent: IsCurrent): HTMLElement {
  ...
  const weekInput = element("input") as HTMLInputElement;
  weekInput.type = "date";
  weekInput.value = report.week_start;
  week.append(weekInput);
  const load = element("button", "button", "Carregar semana") as HTMLButtonElement;
  load.type = "submit";
  ...
  history.addEventListener("submit", (event) => {
    ...
    void (async () => {
      load.disabled = true;
      try {
        const selected = await apiGet<WeeklyReport>(`/reports/weekly/${weekInput.value}`);
...
export async function renderReports(root: HTMLElement, isCurrent: IsCurrent = () => true): Promise<void> {
  if (isCurrent()) root.replaceChildren(element("p", "empty-state", "Carregando relatorio..."));
  try {
    const report = await apiGet<WeeklyReport>("/reports/weekly");
    if (!isCurrent()) return;
