...
import type { Reward, RewardCreate } from "./types.js";
...
function createRewardList(root: HTMLElement, rewards: Reward[], isCurrent: IsCurrent): HTMLElement {
  ...
  if (rewards.length === 0) section.append(element("p", "empty-state", "Nenhuma recompensa ativa."));
...
export async function renderRewards(root: HTMLElement, isCurrent: IsCurrent = () => true): Promise<void> {
  ...
  try {
    const rewards = await apiGet<Reward[]>("/rewards");
    if (!isCurrent()) return;
