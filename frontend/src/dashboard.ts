...
import type { BadgeStatus, Dashboard, Mission, PlayerSummary } from "./types.js";
...
function createStat(label: string, value: string, valueClassName?: string): HTMLElement {
...
function createMissionList(missions: Mission[]): HTMLElement {
...
function createXpProgress(player: PlayerSummary): HTMLElement {
  const progress = createElement("div", "xp-progress");
  const label = createElement(
    "span",
    "stat-label",
    `${player.xp_into_level} / ${player.xp_for_next_level} XP para o proximo nivel`,
  );
...
function createBadge(badge: BadgeStatus | null): HTMLElement {
...
function renderDashboardData(root: HTMLElement, dashboard: Dashboard): void {
  ...
  summary.append(
    createStat("Nivel", String(dashboard.player.level)),
    createStat("XP total", String(dashboard.player.total_xp)),
    createStat("Ouro", String(dashboard.player.gold), "gold"),
    createStat("Streak atual", `${dashboard.player.current_streak} dias`),
    createStat("Melhor streak", `${dashboard.player.best_streak} dias`),
  );
  page.append(summary, createXpProgress(dashboard.player));

  const lowerGrid = createElement("section", "dashboard-grid");
  lowerGrid.append(
    createPanel("Proximas missoes", createMissionList(dashboard.upcoming_missions)),
    createPanel("Hoje", (() => {
      const today = createElement("div", "today-grid");
      today.append(
        createStat("Concluidas", String(dashboard.today.completed)),
        createStat("Ativas", String(dashboard.today.active)),
        createStat("Atrasadas", String(dashboard.today.overdue)),
      );
      return today;
    })()),
    createPanel("Semana", (() => {
      ...
      weekly.append(
        createStat("Missoes", String(dashboard.weekly.missions_completed)),
        createStat("XP ganho", String(dashboard.weekly.xp_gained)),
        createStat("Ouro ganho", String(dashboard.weekly.gold_gained), "gold"),
        createStat("Melhor dia", dashboard.weekly.best_day ?? "Sem dados"),
      );
      return weekly;
    })()),
